...
function validarRG(rg) {
  var rgSemPontos = rg.replace(/\./g, '').replace('-', '');
  if (rgSemPontos.length !== 9) {
    return false;
  }
  var soma = 0;
  for (var i = 0; i < 8; i++) {
    soma += parseInt(rgSemPontos.charAt(i)) * (2 + i);
  }
  var digitoVerificador = 11 - (soma % 11);
  if (digitoVerificador === 10 || digitoVerificador === 11) {
    digitoVerificador = 0;
  }
  return digitoVerificador === parseInt(rgSemPontos.charAt(8));
}
